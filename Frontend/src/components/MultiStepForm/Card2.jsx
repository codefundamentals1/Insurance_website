...
function Card2(props) {
    ...
    const selectBtn = (plan) => {
        setSelectedPlan(plan);
        props.handleMode(slider);
    ...
    const handleNextClick = () => {
        if (selectedPlan === null) alert("Select a plan.");
    ...
    return props.status === 2 ? (
        <div>
            <div className="card2">
                ...
                <button className={`step2Btn ${selectedPlan === "Arcade" ? "step2BtnSelected" : "step2Btn"}`} onClick={() => selectBtn("Arcade")}>
                    ...
                    <h4>Arcade</h4>
                    <p>{slider === 1 ? "$9/mo" : "$90/yr"}</p>
                    {slider === 2 && (<p id="step2Offer">2 months free</p>)}
                </button>
                <button className={`step2Btn ${selectedPlan === "Advanced" ? "step2BtnSelected" : "step2Btn"}`} onClick={() => selectBtn("Advanced")}>
                    ...
                    <h4>Advanced</h4>
                    <p>{slider === 1 ? "$12/mo" : "$120/yr"}</p>
                    {slider === 2 && (<p id="step2Offer">2 months free</p>)}
                </button>
                <button className={`step2Btn ${selectedPlan === "Pro" ? "step2BtnSelected" : "step2Btn"}`} onClick={() => selectBtn("Pro")}>
                    ...
                    <h4>Pro</h4>
                    <p>{slider === 1 ? "$15/mo" : "$150/yr"}</p>                    
                    {slider === 2 && (<p id="step2Offer">2 months free</p>)}
                </button>
                <div className="sliderParent">
                    <h4 className="sliderLabel">Monthly</h4>
                    <div class="slider">
                        <div id="sliderHandle" onClick={handleSliderClick} style={{ left: slider === 1 ? "0%" : "76%" }}></div>
                    </div>
                    <h4 className="sliderLabel">Yearly</h4>
                </div>

                <button className="goBackBtnDesktop" onClick={props.onGoBackClick}>Go Back</button>
                <button className="nextBtnDesktop" onClick={handleNextClick}>Next Step</button>
